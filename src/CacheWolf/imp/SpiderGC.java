...
public class SpiderGC {
	...
	public void doItAlongARoute() {
		...
		while (nextPos != null) {
			...
			if (nextPos != null) {
				origin = startPos;
				pref.log("m: do " + pointsIndex + " of (" + points.size() + ") at " + origin);
				...
				for (int i = 1; i < anzCheckPoints; i++) {
					...
					pref.log("s: do " + pointsIndex + " of (" + points.size() + ") at " + origin);
					getCaches(lateralDistance);
	...
	private int getCacheByWaypointName(CacheHolder ch, boolean isUpdate, boolean fetchImages, boolean fetchTBs, boolean doNotGetFound, boolean fetchAllLogs) {
		...
		while (true) { // retry even if failure
			...
			if ((spiderTrys >= MAX_SPIDER_TRYS) && (ret == SPIDER_OK)) {
				pref.log(">>> Failed to spider cache. Number of retries exhausted.", null);
				final int decision = (new MessageBox(MyLocale.getMsg(5500, "Error"), MyLocale.getMsg(5515, "Failed to load cache.%0aPlease check your internet connection.%0aRetry?"), FormBase.DEFOKB | FormBase.NOB | FormBase.CANCELB))
						.execute();
