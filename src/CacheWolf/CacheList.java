...
public class CacheList extends CellPanel {
	...
	public void onKeyEvent(KeyEvent ev) {
		...
		ev.target = Global.mainTab.tablePanel.myTableControl;
		postEvent(ev);
	    }
	...
	if (ev instanceof MenuEvent && ev.type == MenuEvent.SELECTED) {
		...
		int idx = Global.profile.cacheDB.getIndex(ch);
		...
		if (activeTab == 0) {
		    ...
		    Global.mainTab.tablePanel.selectRow(idx);
		    Global.mainTab.tablePanel.myTableControl.repaint();
		} else {
	...
	if (ev instanceof ControlEvent && ev.type == ControlEvent.PRESSED) {
		...
		FileChooser fc = new FileChooser(FileChooserBase.SAVE, Global.profile.dataDir);
		//fc.addMask(currCh.wayPoint + ".wl");
		fc.addMask("*." + EXTENSION);
		fc.setTitle(MyLocale.getMsg(191, "Select File"));
		if (fc.execute() != FormBase.IDCANCEL) {
	...
	Global.profile.setFilterActive(Filter.FILTER_CACHELIST);
	...
	//		Global.mainTab.tablePanel.myMod.numRows=numRows;
	Global.mainTab.tablePanel.refreshTable(); // Update and repaint
	int selPanel;
	if ((selPanel = Global.mainTab.cardPanel.selectedItem) > -1) {
		...
		Global.mainTab.detailsPanel.repaint();
	    }
	}
    }
	...
	try {
		...
		// Only add the cache if it is in this Global.profile
		CacheHolder ch = Global.profile.cacheDB.get(wayPoint);
		if (ch != null) {
		    addCache(ch);
		    if (selected)
	    ...
	    Global.pref.log("Problem reading: " + fileName, e, true);
	}
	if (select > -1)
	...
	try {
	    ...
	    Global.pref.log("Problem saving: " + fileName, e, true);
	}
	setTitle(fileName);
	dirty = false;
    }
