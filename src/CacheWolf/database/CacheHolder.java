...
public class CacheHolder {
     ...
     *
     * @return <code>true</code> if the waypoint should appear in the cache list.<br>
     */
    ...
    public String getDistance() {
        ...
        if (this.kilom == this.lastKilom && Preferences.itself().metricSystem == this.lastMetric) {
            ...
            if (this.kilom >= 0) {
                ...
                result = Common.formatDouble(newValue, "0.00") + " " + newUnit;
            } else {
     ...
     *
     * @param isNewCache <code>true</code> if it is a new cache (i.e. a cache not existing in CacheDB), <code>false</code> otherwise.
     */
        ...
        if (isOC()) {
            ...
            if (Preferences.itself().useGCFavoriteValue) {
                ...
                if (gcVote < 100) {
                    ...
                    return Common.formatDouble((double) gcVote / 10.0, "0.0");
                } else {
                    int votes = gcVote / 100; // Anzahl Stimmen
                    gcVote = gcVote - 100 * votes; // Durchschnittswert der Abstimmung 1, 1.5 ... 4.5, 5
                    return Common.formatDouble((double) gcVote / 10.0, "0.0") + " (" + Convert.formatInt(votes) + ")";
                }
            }
        }
    }
     ...
     *
     * @param newCode new waypoint id (will be converted to upper case)
