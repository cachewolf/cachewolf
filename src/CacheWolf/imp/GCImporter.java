...
public class GCImporter {
    ...
    public void doItAlongARoute() {
        ...
        // Load �ber die Kreise, daher Faktor 1.2
    ...
    private double examineCacheDescriptionOfListPage(double fromDistance, double toDistance) {
        ...
        if (distance >= fromDistance) { // finds have distance 0
            if (distance <= toDistance) {
                ...
                if (Preferences.itself().havePremiumMemberRights || !isPremiumOnly || true) {
                    if (ch == null) { // not in DB
                        downloadList.add(chWaypoint);
                    }
                    else {
                    ...
                    // ein PremiumCache f�r ein BasicMember
                    if (ch == null) {
                        //downloadList.add(chWaypoint);
    ...
    private String getDateHidden (JSONObject cacheDescription){
        try{
            ...
            return DateFormat.toYYMMDD(dateString);
        }
        catch (JSONException e){
            throw new RuntimeException(e);
        }
        ...
        do {
            ...
            switch (checkGCSettings()) {
                ...
                case 5:
                    break;
                case 6:
                    // no correct login (Passwort falsch eingegeben oder �hnliches)
                ...
                if (ret == SPIDER_OK) {
                    ...
                    if (infB.isClosed()){
                        ...
                        if (!Preferences.itself().havePremiumMemberRights) {
                            ...
                            if (Preferences.itself().addPremiumGC && !isInDB){
                                MainForm.profile.cacheDB.add(newCache);
                                chOld = newCache;
                                isInDB = true;
                            }
    ...
    private CacheImage spiderImage(String wayPoint, String originalUrl, String imgType) {
        ...
        if (idxUrl < 0) {
            ...
            spiderCounter++;
            cacheImage.setComment(""); // eigentlich �berfl�ssig wegen constructor
        } else {
