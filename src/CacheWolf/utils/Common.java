...
public final class Common {
    ...
    public static String formatDouble(ewe.sys.Double number, String fmt) {
        return MyLocale.getLocale().format(ewe.sys.Locale.FORMAT_PARSE_NUMBER, number, fmt);
    }
    ...
    public static String formatDouble(double number, String fmt) {
        return formatDouble(new ewe.sys.Double().set(number), fmt);
    }
    ...
    */
    ...
    /**
     * removes the Extension (everything behind last dot)
     *
     * @param fn
    ...
    public static String fixSerialPortName(String name) {
        if (name.startsWith("/"))
            return ".." + name; // on linux (*nix) machines it is quite usual to give the complete file path to the serial port, but ewe expects only "ttyS0" or similar
        else
            return name;
    }
