...
public final class GotoPanel extends CellPanel {
    ...
    public void updateGps(int fix) {
        ...
        if ((fix > 0) && (Navigate.gpsPos.getSats() >= 0)) {
            ...
            speed = Navigate.gpsPos.getSpeed();
            sunAzimut = MainTab.itself.navigate.skyOrientationDir.lonDec;
            bearMov = Navigate.gpsPos.getBear();
            updateDistance();
            compassRose.setSunMoveDirections((float) sunAzimut, (float) bearMov, (float) speed);
    ...
    public void startGps() {
        MainTab.itself.navigate.startGps(Preferences.itself().logGPS, Common.parseInt(Preferences.itself().logGPSTimer));
    }
...
class GotoRose extends AniImage {
    ...
    private void drawWayPointData(Graphics g) {
        ...
        if (newDistance >= 0.0f) {
            if (newDistance >= threshold) {
                strTempVal = Common.formatDouble(newDistance, "0.000");
                strTemp = strTempVal + " " + Metrics.getUnit(bigUnit);
            } else {
                strTempVal = Common.DoubleToString(Metrics.convertUnit(newDistance, bigUnit, smallUnit), 3, 0);
        ...
        if ((gotoDir <= 360) && (gotoDir >= -360))
            strTemp = Common.DoubleToString(gotoDir, 0, 0) + " " + MyLocale.getMsg(1502, "deg");
        else
    ...
    private void drawGpsData(Graphics g) {
        ...
        if (Preferences.itself().metricSystem == Metrics.IMPERIAL) {
            tmp = Metrics.convertUnit(m_speed, Metrics.KILOMETER, Metrics.MILES);
            unit = " mph";
            strSpeed = "- mph";
        } else {
            tmp = m_speed;
        ...
        if (tmp >= 0) {
            if (tmp >= 100) {
                strSpeed = Common.formatDouble(tmp, "0") + unit;
            } else {
                strSpeed = Common.formatDouble(tmp, "0.0") + unit;
            }
        }
        ...
        if ((tmp <= 360) && (tmp >= -360))
            strMoveDir = Common.DoubleToString(tmp, 0, 0) + " " + MyLocale.getMsg(1502, "deg");
    ...
    private void drawLuminaryData(Graphics g) {
        ...
        if (sunDir < 360 && sunDir > -360) {
            strSunDir = Common.DoubleToString(sunDir, 0, 0) + " " + MyLocale.getMsg(1502, "deg");
        }
    ...
    private void drawGpsStatus(Graphics g) {
        ...
        String strSats = "Sats: -";
        if (m_sats >= 0) {
            strSats = "Sats: " + m_sats + "/" + m_satsInView;
        }
        String strHdop = "HDOP: -";
        if (m_hdop >= 0)
            strHdop = "HDOP: " + m_hdop;
