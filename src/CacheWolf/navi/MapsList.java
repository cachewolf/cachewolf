...
public final class MapsList extends Vector {
     ...
     *
     * @param lat only for adding empty maps
     */
     ...
     *
     * @param ll              CWPoint: ll.latDec/ll.lonDec a point to be inside the map
     * @param screen          Rect: width, height of the screen. The map must overlap the screen.
     * @param scale           float: scale wanted.
     * @param forceScale      : when true, return null if no map with specified scale could be found
     * @param withProgressBox boolean: true -> with ProgressBox
     ...
     * @param ulimit     upper limit to stop the search. llimit and ulimit must be set in a way that the sortEntryBBox of each entry betwenn the limits have the same length
    ...
    /**
     * @param ll          lat/lon a point to be inside the map
     ...
     *
     * @param screen screen width/height and x/y position on the screen of lat/lon
     * @param ll     lat/lon a (reference) point on the screen
