...
public class MyTableControl extends TableControl {
    private static final int BLACKLISTED = 0;
        ...
        if (menuItems == null) {
            if (Preferences.itself().hasTickColumn) {
                menuItems = new MenuItem[]{miSetDestination, miCenter, miUnhideAddis, miSeparator, //
                        ...
                        miDelete, miUpdate, miChangeBlacklisted, miChangeSolved, miSeparator, //
    ...
    public void popupMenuEvent(Object selectedItem) {
        ...
        } else if (selectedItem == miChangeBlacklisted) {
            changeStatus(BLACKLISTED);
        } else if (selectedItem == miChangeSolved) {
            changeStatus(SOLVED);
        } else if (selectedItem == miSetDestination) {
